...
def post_genre_type_page(request, id):
    ...
    story_idea_form = StoryIdeaForm(request.POST)

    if story_idea_form.is_valid():
        story_idea_form.save()
        return redirect('get_my_stories_page', id = user.id)
